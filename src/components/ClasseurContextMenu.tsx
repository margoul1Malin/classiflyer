...
import { Edit3, Archive, Trash2, MoreVertical } from 'lucide-react';
import { Classeur } from '../types';

interface ClasseurContextMenuProps {
  isOpen: boolean;
  onClose: () => void;
  position: { x: number; y: number };
  classeur: Classeur;
  onEdit: (classeur: Classeur) => void;
  onArchive: (classeur: Classeur) => void;
  onDelete: (classeur: Classeur) => void;
}

const ClasseurContextMenu: React.FC<ClasseurContextMenuProps> = ({
  isOpen,
  onClose,
  position,
  classeur,
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 flex items-center space-x-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
...
export default ClasseurContextMenu;
